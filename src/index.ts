...
/*
let objData = {
  enc_key: process.env.ENC_KEY || "bf3c199c2470cb477d907b1e0917c17b",
  iv: process.env.IV_KEY || "5183666c72eec9e4",
  status_product:process.env.PRODUCTION || "beta",
  jwtKey: process.env.JWT_KEY || "ThanhNv",
  jwtKeyTime: process.env.JWT_KEY_TIME,
  jwtReKey: process.env.JWT_REKEY || "ThanhNv",
  jwtReKeyTime: process.env.JWT_REKEY_TIME || 14400,
}
*/
...
const _generateAuthToken = (inpVal: any, objData:any) => {
...
const _getDefaultToken = (inValue: string = "nguoihanoi") => {
  return _cryptoMd5(inValue + "@" + new Date().toISOString().split("T")[0]);
};

const _encryptAES = (inpVal: string, objData:any) => {
...
const _decryptAES = (inpVal: string, objData:any) => {
...
export const encryptAES = (inpVal: string, objData:any) => {
  return _encryptAES(inpVal, objData);
};

export const decryptAES = (inpVal: string, objData:any) => {
  return _decryptAES(inpVal, objData);
};
...
export const generateAuthToken = (inpVal: any, objData:any) => {
  return _generateAuthToken(inpVal, objData);
}

export const generateAuthRefreshToken = (inpVal: string, objData:any) => {
  // Generate an auth token for the user
  let token = "";
  if (objData.status_product != "live") {
